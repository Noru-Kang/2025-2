...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        # 음식 위치 : 가까울수록 증가 - 멘해튼 둘 다 사용하자 / 남은거 개수? 남은거는 오히려 점수를 하락
        # 유령 위치 : 멀수록 증가 - 멘해튼 둘 다 사용하자(가까울수록 가중치 증가) 나머지는 총 합
        # 가까운 유령은 따로 계산
            # 겁먹은 상태 : 가까울수록 증가
            # 겁먹지 않은 상태 : 멀수록 증가
        # 파워팰릿 : 유령 먹을 수 있고, 시간제약 존재 - 유령먹으면 점수 추가
        # 파워팰릿에 가까울수록 점수 증가 : 먹으면 점수 높아지도록 / 시간 감소는 주가 비용으로 처리하자(더 cost가 많이 떨어지게)
        # time penalty 존재
        # score는 커지게만하자
        # 스케일링 존재
        # 다항식으로 처리 예정
